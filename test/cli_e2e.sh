...
mkdir -p "$dataset/CJ-HUMANEVAL-000/starter/src" "$dataset/CJ-HUMANEVAL-000/tests" "$artifacts"
...
output=$($cli run --manifest "$manifest" --checksum "$checksum" --root "$artifacts" --repo-root "$repo_root" --local-only --starter-root "$dataset" --tests-root "$dataset" --codex "$repo_root/test/fixtures/fake_codex.sh" --model fake --run-id e2e --case-id CJ-HUMANEVAL-000)
printf '%s\n' "$output"
